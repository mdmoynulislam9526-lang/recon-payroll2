...
# --- TABS ---
        ...
        with tab1:
            selected_emp_id = st.selectbox("Select Employee for Pay Slip", [f"{r['emp_id']} - {r['name']}" for r in rows])
            ...
            # Pay Slip HTML
            payslip_preview_html = f"""<div style="font-family: 'Segoe UI', sans-serif; padding: 35px; background: white; color: black; border: 1px solid #ccc; border-radius: 8px;">
                <div style="text-align: center; border-bottom: 3px solid #1F4E78;"><h3>Employee Pay Slip</h3></div>
                <p>Name: {selected_emp['name']} | ID: {selected_emp['emp_id']}</p>
                <p>Net Payable: Tk {net_final:,.2f}</p>
            </div>"""
            ...
            st.download_button("📥 Download Pay Slip (PDF)", data=pdf_buf.getvalue(), file_name=f"PaySlip_{selected_id}.pdf", mime="application/pdf")
